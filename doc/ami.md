...
- CQサーバのデータボリュームのバックアップのスケジュールを設定します。
  - AWSマネジメントコンソールでEC2にアクセスし、「ライフサイクルマネージャー」＞「スナップショットライフサイクルポリシーの作成」をクリックします。
    ...
    - ポリシースケジュール
      - スケジュール名: cq-data-volume-backup-schedule
      - ポリシーの実行間隔: 24時間
    ...
    - ポリシー
      - 作成後のポリシーのステータス: 有効化
    - 「ポリシーの作成」をクリックします。
- CIサーバのデータボリュームのバックアップのスケジュールについても同じ操作で設定します。  
  CQサーバと値を変える部分について以下に示します。
  - 説明: ci-data-volume-backup
  - これらのタグを持つターゲット: Name:nop-ebs-data-ci
  - ポリシースケジュール
    - スケジュール名: ci-data-volume-backup-schedule
